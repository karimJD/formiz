 ...
 */
export const isMaxNumber = (max: number) => (value: Value): boolean =>
  testIsEmpty(value) || (testIsNumber(value) && parseFloat(value) <= max);

/**
 * Check if the value is between the minimum and the maximum.
 */
export const isInRangeNumber = (min: number, max: number) => (
  value: Value,
): boolean =>
  testIsEmpty(value) ||
  (testIsNumber(value) && parseFloat(value) >= min && parseFloat(value) <= max);

/**
 * Check if the value is a percentage, between 0 and 100.
 */
export const isPercentage = () => (value: Value): boolean =>
  testIsEmpty(value) ||
 ...
 */
export const isMaxLength = (max: number) => (value: Value): boolean =>
  testIsEmpty(value) ||
 ...
 */
export const isPattern = (pattern: string | RegExp) => (
  value: Value,
): boolean => {
  if (testIsEmpty(value)) return true;
